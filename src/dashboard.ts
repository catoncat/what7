...
async function loadStats(){ $('#stats').innerHTML='<div class="stats-note">Loading stats…</div>'; const {summary}=await j('/api/analytics'); $('#stats').innerHTML=[['Sessions',summary.sessionCount],['Messages',summary.messageCount],['Tools',summary.toolCallCount],['Output tok',summary.tokenUsage.outputTokens]].map(([k,v])=>'<div class="card"><b>'+esc(v)+'</b><span>'+esc(k)+'</span></div>').join(''); }
